...
import PrecisionCaseStudy from '@/components/work/PrecisionCaseStudy'
...
export default async function ProjectPage({
  ...
  const panelItems = [
    ...
    ...(project.liveUrl
      ...
      : []),
    ...(project.summary
      ? [{ label: 'Description', value: project.summary }]
      : []),
    ...(project.stack
  ...
  const isPrecision = slug === 'precision'
        ...
        {project.stats && project.stats.length > 0 && (
          ...
          >
            {project.stats.map((stat) => (
              <div
                key={stat.label}
                style={{ backgroundColor: 'var(--bg-base)', padding: '24px 16px' }}
              >
                <div style={{ fontSize: '24px', color: 'var(--text-primary)', fontWeight: 400, marginBottom: '4px' }}>
                  {stat.number}
                </div>
          ...
          <div style={{ marginBottom: '48px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {project.backedBy.map((org) => (
                <div key={org} style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>
                  {org}
                </div>
              ))}
            </div>
